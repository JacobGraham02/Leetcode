...
public class LowestCommonAncestorOfBST {
    ...
    public BinaryTreeNode lowestCommonAncestorIterative(BinaryTreeNode root, BinaryTreeNode p, BinaryTreeNode q) {
        ...
        final int maximumCommonAncestorValue = Math.max(p.val, q.val);
        final int minimumCommonAncestorValue = Math.min(p.val, q.val);

        while (root != null) {
            if (root.val > maximumCommonAncestorValue) {
                root = root.left;
            } else if (root.val < minimumCommonAncestorValue) {
                root = root.right;
            } else {
                return root;
            }
